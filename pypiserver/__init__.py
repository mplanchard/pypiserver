...
def app(config=None, validate_config=True, **kwds):
    """
    :param core.Configuration config: optional configuration object to
        use for the app.
    ...
    if validate_config:
        config.validate()
    _app.config = config
    _app.packages = core.Packages(config)
...
def paste_app_factory(global_config, validate_config=True, **local_conf):
    import os
    ...
    if not validate_config:
        c['validate_config'] = False

    return app(**c)


def logwrite(logger, level, msg):
    if msg:
        line_endings = ['\r\n', '\n\r', '\n']
